...
const typeColors: Record<string, string> = {
  ...
  ERROR_FIX: "text-red-400 bg-red-900/30",
  PATTERN: "text-blue-400 bg-blue-900/30",
  GOTCHA: "text-orange-400 bg-orange-900/30",
  PREFERENCE: "text-purple-400 bg-purple-900/30",
  DECISION: "text-yellow-400 bg-yellow-900/30",
}

const agentColors: Record<string, string> = {
  dev: "text-cyan-400 bg-cyan-900/30",
  ops: "text-emerald-400 bg-emerald-900/30",
  sales: "text-pink-400 bg-pink-900/30",
  research: "text-violet-400 bg-violet-900/30",
  main: "text-amber-400 bg-amber-900/30",
  academic: "text-teal-400 bg-teal-900/30",
  finance: "text-lime-400 bg-lime-900/30",
}
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-3">
        <div className="card">
          <div className="text-xs text-hud-muted">Total Learnings</div>
          <div className="text-2xl font-bold text-green-400">{learnings.length}</div>
        </div>
        <div className="card">
          <div className="text-xs text-hud-muted">Today</div>
          <div className="text-2xl font-bold text-blue-400">{todayCount}</div>
        </div>
        <div className="card">
          <div className="text-xs text-hud-muted">Active Agents</div>
          <div className="text-2xl font-bold text-purple-400">{agents.length}</div>
        </div>
        <div className="card">
          <div className="text-xs text-hud-muted">Top Learner</div>
        ...
        {([
          ...
          <button
            ...
            className={`px-4 py-2 rounded-lg text-sm font-medium transition-colors ${
              tab === t ? "bg-hud-amber text-space-black" : "bg-space-panel text-hud-muted hover:bg-space-panel"
            }`}
          >
            {label}
          </button>
          ...
          <div className="space-y-6">
            {grouped.map((group) => (
              <div key={group.date}>
                <div className="flex items-center gap-3 mb-3">
                  <div className="text-sm font-semibold text-hud-text">{group.label}</div>
                  <div className="flex-1 h-px bg-space-border" />
                  <div className="text-xs text-hud-muted">{group.items.length} learnings</div>
                </div>
                <div className="space-y-2 ml-4 border-l-2 border-space-border pl-4">
                  {group.items.map((l) => (
                    <div key={l.id} className="card hover:border-space-border transition-colors cursor-pointer" onClick={() => l.context && toggleExpand(l.id)}>
                      <div className="flex flex-wrap items-center gap-2 mb-2">
                        <span className={`px-2 py-0.5 rounded text-xs font-medium ${typeColors[l.type] || "text-hud-muted bg-space-border"}`}>{l.type}</span>
                        <span className={`px-2 py-0.5 rounded text-xs font-medium ${agentColors[l.agent] || "text-hud-muted bg-space-border"}`}>{l.agent}</span>
                        {l.category && <span className="text-xs text-hud-muted">#{l.category}</span>}
                        <span className="text-xs text-hud-muted/70 ml-auto">{new Date(l.created_at).toLocaleTimeString("en-US", { hour: "numeric", minute: "2-digit" })}</span>
                      </div>
                      {l.title && <div className="font-medium text-sm mb-1">{l.title}</div>}
                      <div className="text-sm text-hud-text">{l.content}</div>
                      {l.tags && l.tags.length > 0 && (
                        <div className="flex gap-1 mt-2">{l.tags.map((tag) => <span key={tag} className="text-xs px-1.5 py-0.5 bg-space-border rounded text-hud-muted">{tag}</span>)}</div>
                      )}
                      {l.context && expandedIds.has(l.id) && (
                        <div className="mt-3 pt-3 border-t border-space-border text-xs text-hud-muted"><span className="text-hud-muted font-medium">Context:</span> {l.context}</div>
                      )}
                      {l.context && !expandedIds.has(l.id) && <div className="text-xs text-hud-muted/70 mt-2">▸ Click to show context</div>}
                    </div>
                  ))}
                </div>
              </div>
            ...
            {filtered.length === 0 && (
              <div className="card text-center py-16">
                ...
                <div className="text-hud-muted">No learnings yet</div>
                <div className="text-hud-muted text-sm mt-2">Agents capture learnings automatically as they work</div>
              </div>
            )}
          </div>
        </>
      ...
      {tab === "howItWorks" && (
        <div className="space-y-6">
          ...
          <div className="card">
            ...
            <p className="text-hud-text mb-4">
            ...
            <div className="p-4 bg-space-panel rounded-lg text-sm text-hud-text font-mono leading-relaxed">
              <div className="text-hud-muted mb-2">{"// The architecture in plain english:"}</div>
          ...
          <div className="card">
            ...
            <div className="space-y-4">
              <div className="p-4 bg-space-panel rounded-lg border-l-4 border-blue-500">
                <h3 className="font-bold text-blue-400 mb-1">1. The Brain — OpenClaw</h3>
                <p className="text-sm text-hud-text mb-2">
              ...
              <div className="p-4 bg-space-panel rounded-lg border-l-4 border-green-500">
              ...
              <div className="p-4 bg-space-panel rounded-lg border-l-4 border-purple-500">
              ...
              <div className="p-4 bg-space-panel rounded-lg border-l-4 border-yellow-500">
              ...
              <div className="p-4 bg-space-panel rounded-lg border-l-4 border-cyan-500">
              ...
              <div className="p-4 bg-space-panel rounded-lg border-l-4 border-pink-500">
          ...
          <div className="card">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              <div className="p-4 bg-space-panel rounded-lg">
                <h3 className="font-bold text-sm mb-2">When you send a message</h3>
                <ol className="text-xs text-hud-muted space-y-1 list-decimal list-inside">
          ...
          <div className="card">
            ...
            <div className="space-y-2 text-xs">
              {[
                ...
                <div key={ep.path + ep.method} className="p-3 bg-space-panel rounded-lg font-mono">
                  <span className={ep.method === "POST" ? "text-green-400" : "text-cyan-400"}>{ep.method}</span>{" "}
                  <span className="text-blue-400">{ep.path}</span>
                  <span className="text-hud-muted ml-2">— {ep.desc}</span>
                  <div className="text-hud-muted/70 mt-1">{ep.body}</div>
                </div>
              ))}
            </div>
          </div>
        ...
        <div className="space-y-6">
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
            <div className="card">
              ...
              <ul className="space-y-3 text-sm text-hud-muted">
            ...
            <div className="card">
              ...
              <ul className="space-y-3 text-sm text-hud-text">
          ...
          <div className="card">
            ...
            <div className="space-y-4">
              {[
                ...
                <div key={item.title} className={`p-4 bg-space-panel rounded-lg border-l-4 border-${item.color}-500`}>
                  <h3 className="font-bold mb-1">{item.title}</h3>
                  <p className="text-sm text-hud-text">{item.what}</p>
                </div>
              ))}
            </div>
          </div>

          <div className="card bg-gradient-to-r from-space-panel to-space-dark">
          ...
          <div className="card">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              {[
                ...
                <div key={c.term} className="p-4 bg-space-panel rounded-lg">
                  <h3 className="font-bold text-hud-text mb-1">{c.term}</h3>
                  <p className="text-sm text-hud-text mb-2">{c.def}</p>
                  <p className="text-xs text-hud-muted italic">Example: {c.example}</p>
                </div>
              ))}
            </div>
          </div>
          ...
          <div className="card">
            ...
            <div className="space-y-3">
              {[
                ...
                <div key={s.step} className="flex gap-4 p-4 bg-space-panel rounded-lg">
                  <div className="text-2xl font-bold text-blue-400 w-8 shrink-0">{s.step}</div>
                  <div>
                    <h3 className="font-bold text-hud-text mb-1">{s.title}</h3>
                    <p className="text-sm text-hud-muted">{s.detail}</p>
                  </div>
                </div>
              ))}
            </div>
          ...
          <div className="card">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
              {[
                ...
                <div key={p.principle} className="p-3 bg-space-panel rounded-lg">
                  <h3 className="font-bold text-sm text-hud-text mb-1">{p.principle}</h3>
                  <p className="text-xs text-hud-muted">{p.detail}</p>
                </div>
              ))}
            </div>
          </div>
          ...
          <div className="card">
            ...
            <div className="space-y-2">
              {[
                ...
                <div key={m.mistake} className="flex gap-3 p-3 bg-space-panel rounded-lg">
                  <span className="text-red-400 shrink-0">✗</span>
                  <div>
                    <span className="text-hud-text text-sm">{m.mistake}</span>
              ...
              <div key={agent.id} className="card">
                <div className="flex items-center gap-3 mb-4">
                  ...
                  <div>
                    ...
                    <div className="text-sm text-hud-muted">{agent.role}</div>
                  </div>
                  <div className="ml-auto text-right">
                    <div className="text-2xl font-bold text-green-400">{agentLearnings.length}</div>
                    <div className="text-xs text-hud-muted">learnings</div>
                  </div>
                </div>
                <p className="text-sm text-hud-text mb-4 italic">{agent.personality}</p>
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  <div>
                    <h3 className="text-xs font-bold text-hud-muted uppercase mb-2">Responsibilities</h3>
                    <ul className="space-y-1">
                      {agent.responsibilities.map((r) => (
                        <li key={r} className="text-sm text-hud-text flex gap-2">
                          <span className="text-hud-muted/70">→</span> {r}
                        </li>
                      ))}
                    </ul>
                  </div>
                  <div>
                    <h3 className="text-xs font-bold text-hud-muted uppercase mb-2">Tools</h3>
                    <div className="flex flex-wrap gap-1 mb-4">
                      {agent.tools.map((t) => (
                        <span key={t} className="text-xs px-2 py-1 bg-space-panel rounded text-hud-muted">{t}</span>
                      ))}
                    </div>
                    <h3 className="text-xs font-bold text-hud-muted uppercase mb-2">Memory</h3>
                    <p className="text-xs text-hud-muted">{agent.memory}</p>
                  </div>
                </div>
                <div className="mt-4 pt-3 border-t border-space-border text-sm text-hud-muted italic">{agent.motto}</div>
              </div>
            )
          })}
        </div>
