...
const typeColors: Record<string, string> = {
  ...
  ERROR_FIX: "text-red-600 bg-red-50",
  PATTERN: "text-blue-600 bg-blue-50",
  GOTCHA: "text-orange-600 bg-orange-50",
  PREFERENCE: "text-purple-600 bg-purple-50",
  DECISION: "text-yellow-600 bg-yellow-50",
}

const agentColors: Record<string, string> = {
  dev: "text-cyan-600 bg-cyan-50",
  ops: "text-emerald-600 bg-emerald-50",
  sales: "text-pink-600 bg-pink-50",
  research: "text-violet-600 bg-violet-50",
  main: "text-amber-600 bg-amber-50",
  academic: "text-teal-600 bg-teal-50",
  finance: "text-lime-600 bg-lime-50",
}
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-3">
        <div className="card">
          <div className="text-xs text-warm-muted">Total Learnings</div>
          <div className="text-2xl font-bold text-green-600">{learnings.length}</div>
        </div>
        <div className="card">
          <div className="text-xs text-warm-muted">Today</div>
          <div className="text-2xl font-bold text-blue-600">{todayCount}</div>
        </div>
        <div className="card">
          <div className="text-xs text-warm-muted">Active Agents</div>
          <div className="text-2xl font-bold text-purple-600">{agents.length}</div>
        </div>
        <div className="card">
          <div className="text-xs text-warm-muted">Top Learner</div>
          <div className="text-2xl font-bold text-yellow-600">{topAgent}</div>
        </div>
      </div>
        ...
        {([
          ...
          <button
            ...
            className={`px-4 py-2 rounded-lg text-sm font-medium transition-colors ${
              tab === t ? "bg-blue-600 text-white" : "bg-cream-100 text-warm-muted hover:bg-cream-200"
            }`}
          >
            {label}
          </button>
          ...
          <div className="space-y-6">
            {grouped.map((group) => (
              <div key={group.date}>
                <div className="flex items-center gap-3 mb-3">
                  <div className="text-sm font-semibold text-warm-text">{group.label}</div>
                  <div className="flex-1 h-px bg-cream-200" />
                  <div className="text-xs text-warm-muted">{group.items.length} learnings</div>
                </div>
                <div className="space-y-2 ml-4 border-l-2 border-cream-200 pl-4">
                  {group.items.map((l) => (
                    <div key={l.id} className="card hover:border-cream-200 transition-colors cursor-pointer" onClick={() => l.context && toggleExpand(l.id)}>
                      <div className="flex flex-wrap items-center gap-2 mb-2">
                        <span className={`px-2 py-0.5 rounded text-xs font-medium ${typeColors[l.type] || "text-warm-muted bg-cream-200"}`}>{l.type}</span>
                        <span className={`px-2 py-0.5 rounded text-xs font-medium ${agentColors[l.agent] || "text-warm-muted bg-cream-200"}`}>{l.agent}</span>
                        {l.category && <span className="text-xs text-warm-muted">#{l.category}</span>}
                        <span className="text-xs text-warm-muted/70 ml-auto">{new Date(l.created_at).toLocaleTimeString("en-US", { hour: "numeric", minute: "2-digit" })}</span>
                      </div>
                      {l.title && <div className="font-medium text-sm mb-1">{l.title}</div>}
                      <div className="text-sm text-warm-text">{l.content}</div>
                      {l.tags && l.tags.length > 0 && (
                        <div className="flex gap-1 mt-2">{l.tags.map((tag) => <span key={tag} className="text-xs px-1.5 py-0.5 bg-cream-200 rounded text-warm-muted">{tag}</span>)}</div>
                      )}
                      {l.context && expandedIds.has(l.id) && (
                        <div className="mt-3 pt-3 border-t border-cream-200 text-xs text-warm-muted"><span className="text-warm-muted font-medium">Context:</span> {l.context}</div>
                      )}
                      {l.context && !expandedIds.has(l.id) && <div className="text-xs text-warm-muted/70 mt-2">▸ Click to show context</div>}
                    </div>
                  ))}
                </div>
              </div>
            ...
            {filtered.length === 0 && (
              <div className="card text-center py-16">
                ...
                <div className="text-warm-muted">No learnings yet</div>
                <div className="text-warm-muted text-sm mt-2">Agents capture learnings automatically as they work</div>
              </div>
            )}
          </div>
        </>
      ...
      {tab === "howItWorks" && (
        <div className="space-y-6">
          {/* The Big Picture */}
          <div className="card">
            <h2 className="text-xl font-bold mb-2">The Big Picture</h2>
            ...
            <div className="p-4 bg-cream-100 rounded-lg text-sm text-warm-text font-mono leading-relaxed">
              <div className="text-warm-muted mb-2">{"// The architecture in plain english:"}</div>
              <div><span className="text-blue-600">You</span> → message on Discord/Telegram</div>
              <div className="ml-4">↓</div>
              <div><span className="text-green-600">OpenClaw Gateway</span> → routes to the right agent</div>
              <div className="ml-4">↓</div>
              <div><span className="text-purple-600">Agent (Tony, etc.)</span> → thinks, uses tools, takes action</div>
              <div className="ml-4">↓</div>
              <div><span className="text-yellow-600">Real-world effect</span> → code pushed, email sent, task created</div>
              <div className="ml-4">↓</div>
              <div><span className="text-cyan-600">Learning captured</span> → agent remembers for next time</div>
            </div>
          </div>
          ...
          <div className="card">
            ...
            <div className="space-y-4">
              <div className="p-4 bg-cream-100 rounded-lg border-l-4 border-blue-500">
                <h3 className="font-bold text-blue-600 mb-1">1. The Brain — OpenClaw</h3>
              ...
              <div className="p-4 bg-cream-100 rounded-lg border-l-4 border-green-500">
                <h3 className="font-bold text-green-600 mb-1">2. The Agents — Specialized Workers</h3>
                <p className="text-sm text-warm-text mb-2">
              ...
              <div className="p-4 bg-cream-100 rounded-lg border-l-4 border-purple-500">
                <h3 className="font-bold text-purple-600 mb-1">3. The Tools — Real Actions</h3>
                <p className="text-sm text-warm-text mb-2">
              ...
              <div className="p-4 bg-cream-100 rounded-lg border-l-4 border-yellow-500">
                <h3 className="font-bold text-yellow-600 mb-1">4. The Memory — Agents That Learn</h3>
                <p className="text-sm text-warm-text mb-2">
              ...
              <div className="p-4 bg-cream-100 rounded-lg border-l-4 border-cyan-500">
                <h3 className="font-bold text-cyan-600 mb-1">5. The Dashboard — Mission Control</h3>
                <p className="text-sm text-warm-text mb-2">
              ...
              <div className="p-4 bg-cream-100 rounded-lg border-l-4 border-pink-500">
                <h3 className="font-bold text-pink-600 mb-1">6. The Knowledge Base — Obsidian</h3>
                <p className="text-sm text-warm-text mb-2">
          ...
          <div className="card">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              <div className="p-4 bg-cream-100 rounded-lg">
                ...
                <ol className="text-xs text-warm-muted space-y-1 list-decimal list-inside">
              ...
              <div className="p-4 bg-cream-100 rounded-lg">
                <h3 className="font-bold text-sm mb-2">When code is built</h3>
                <ol className="text-xs text-warm-muted space-y-1 list-decimal list-inside">
          ...
          <div className="card">
            ...
            <div className="space-y-2 text-xs">
              {[
                ...
                <div key={ep.path + ep.method} className="p-3 bg-cream-100 rounded-lg font-mono">
                  <span className={ep.method === "POST" ? "text-green-600" : "text-cyan-600"}>{ep.method}</span>{" "}
                  <span className="text-blue-600">{ep.path}</span>
                  <span className="text-warm-muted ml-2">— {ep.desc}</span>
                  <div className="text-warm-muted/70 mt-1">{ep.body}</div>
                </div>
              ))}
            </div>
          </div>
      ...
      {tab === "whyDifferent" && (
        <div className="space-y-6">
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
            <div className="card">
              <h3 className="font-bold text-red-600 mb-3">What most people do</h3>
              <ul className="space-y-3 text-sm text-warm-muted">
                <li className="flex gap-2"><span className="text-red-600">•</span> Use ChatGPT for one-off questions</li>
                <li className="flex gap-2"><span className="text-red-600">•</span> Copy-paste code from AI into their editor</li>
                <li className="flex gap-2"><span className="text-red-600">•</span> Manually manage tasks, email, calendar</li>
                <li className="flex gap-2"><span className="text-red-600">•</span> No memory between conversations</li>
                <li className="flex gap-2"><span className="text-red-600">•</span> AI can only talk, can't take action</li>
                <li className="flex gap-2"><span className="text-red-600">•</span> One generic AI for everything</li>
                <li className="flex gap-2"><span className="text-red-600">•</span> Start from scratch every session</li>
              </ul>
            </div>
            <div className="card">
              <h3 className="font-bold text-green-600 mb-3">What we do</h3>
              <ul className="space-y-3 text-sm text-warm-text">
                <li className="flex gap-2"><span className="text-green-600">•</span> Persistent agents that run 24/7 on our hardware</li>
                <li className="flex gap-2"><span className="text-green-600">•</span> Agents write, commit, and deploy code themselves</li>
                <li className="flex gap-2"><span className="text-green-600">•</span> Automated briefings, task management, email triage</li>
                <li className="flex gap-2"><span className="text-green-600">•</span> Memory that compounds — agents learn and remember</li>
                <li className="flex gap-2"><span className="text-green-600">•</span> Real tools: shell, git, APIs, browsers, databases</li>
                <li className="flex gap-2"><span className="text-green-600">•</span> Specialized agents for dev, ops, sales, research</li>
                <li className="flex gap-2"><span className="text-green-600">•</span> Each session builds on everything before it</li>
              </ul>
            </div>
          </div>
          ...
          <div className="card">
            ...
            <div className="space-y-4">
              {[
                ...
                <div key={item.title} className={`p-4 bg-cream-100 rounded-lg border-l-4 border-${item.color}-500`}>
                  <h3 className="font-bold mb-1">{item.title}</h3>
                  <p className="text-sm text-warm-text">{item.what}</p>
                </div>
              ))}
            </div>
          </div>
      ...
      {tab === "playbook" && (
        <div className="space-y-6">
          ...
          <div className="card">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              {[
                ...
                <div key={c.term} className="p-4 bg-cream-100 rounded-lg">
                  <h3 className="font-bold text-warm-text mb-1">{c.term}</h3>
                  <p className="text-sm text-warm-text mb-2">{c.def}</p>
                  <p className="text-xs text-warm-muted italic">Example: {c.example}</p>
                </div>
              ))}
            </div>
          </div>
          ...
          <div className="card">
            <h2 className="text-lg font-bold mb-4">Building an Agent -- Step by Step</h2>
            <div className="space-y-3">
              {[
                ...
                <div key={s.step} className="flex gap-4 p-4 bg-cream-100 rounded-lg">
                  <div className="text-2xl font-bold text-blue-600 w-8 shrink-0">{s.step}</div>
                  <div>
                    <h3 className="font-bold text-warm-text mb-1">{s.title}</h3>
                    <p className="text-sm text-warm-muted">{s.detail}</p>
                  </div>
                </div>
              ))}
            </div>
          ...
          {/* Principles */}
          <div className="card">
            <h2 className="text-lg font-bold mb-4">Principles of Agentic Engineering</h2>
            <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
              {[
                ...
                <div key={p.principle} className="p-3 bg-cream-100 rounded-lg">
                  <h3 className="font-bold text-sm text-warm-text mb-1">{p.principle}</h3>
                  <p className="text-xs text-warm-muted">{p.detail}</p>
                </div>
              ))}
            </div>
          </div>
          ...
          <div className="card">
            ...
            <div className="space-y-2">
              {[
                ...
                <div key={m.mistake} className="flex gap-3 p-3 bg-cream-100 rounded-lg">
                  <span className="text-red-600 shrink-0">✗</span>
                  <div>
                    <span className="text-warm-text text-sm">{m.mistake}</span>
                    <span className="text-warm-muted text-sm"> → </span>
                    <span className="text-green-600 text-sm">{m.fix}</span>
                  </div>
                </div>
              ))}
            </div>
              ...
              <div key={agent.id} className="card">
                <div className="flex items-center gap-3 mb-4">
                  ...
                  <div>
                    ...
                    <div className="text-sm text-warm-muted">{agent.role}</div>
                  </div>
                  <div className="ml-auto text-right">
                    <div className="text-2xl font-bold text-green-600">{agentLearnings.length}</div>
                    <div className="text-xs text-warm-muted">learnings</div>
                  </div>
                </div>
                <p className="text-sm text-warm-text mb-4 italic">{agent.personality}</p>
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  <div>
                    <h3 className="text-xs font-bold text-warm-muted uppercase mb-2">Responsibilities</h3>
                    <ul className="space-y-1">
                      {agent.responsibilities.map((r) => (
                        <li key={r} className="text-sm text-warm-text flex gap-2">
                          <span className="text-warm-muted/70">→</span> {r}
                        </li>
                      ))}
                    </ul>
                  </div>
                  <div>
                    <h3 className="text-xs font-bold text-warm-muted uppercase mb-2">Tools</h3>
                    <div className="flex flex-wrap gap-1 mb-4">
                      {agent.tools.map((t) => (
                        <span key={t} className="text-xs px-2 py-1 bg-cream-100 rounded text-warm-muted">{t}</span>
                      ))}
                    </div>
                    <h3 className="text-xs font-bold text-warm-muted uppercase mb-2">Memory</h3>
                    <p className="text-xs text-warm-muted">{agent.memory}</p>
                  </div>
                </div>
                <div className="mt-4 pt-3 border-t border-cream-200 text-sm text-warm-muted italic">{agent.motto}</div>
              </div>
            )
          })}
        </div>
