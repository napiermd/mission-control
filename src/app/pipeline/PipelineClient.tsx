...
const stages = [
  { id: "IDEA", label: "💡 Ideas", color: "gray", bg: "bg-gray-800", text: "text-gray-300", dot: "bg-gray-400" },
  { id: "SCRIPT", label: "📝 Script", color: "purple", bg: "bg-purple-900/30", text: "text-purple-300", dot: "bg-purple-400" },
  { id: "THUMBNAIL", label: "🖼️ Thumbnail", color: "yellow", bg: "bg-yellow-900/30", text: "text-yellow-300", dot: "bg-yellow-400" },
  { id: "FILMING", label: "🎬 Filming", color: "blue", bg: "bg-blue-900/30", text: "text-blue-300", dot: "bg-blue-400" },
  { id: "PUBLISHED", label: "✅ Published", color: "green", bg: "bg-green-900/30", text: "text-green-300", dot: "bg-green-400" },
]
              ...
              <div className="space-y-2">
                {items.map((item) => (
                  ...
                  >
                    ...
                    {item.platform && (
                      <div className="text-[10px] text-gray-500 mt-1">{platformIcons[item.platform.toLowerCase()] || "📄"} {item.platform}</div>
                    )}
                    {item.notes && <div className="text-[10px] text-gray-600 mt-1 line-clamp-2">{item.notes}</div>}
                    {item.scheduled_date && <div className="text-[10px] text-yellow-500 mt-1">📅 {item.scheduled_date}</div>}
                  </div>
                ))}
                {items.length === 0 && <div className="text-center py-6 text-gray-600 text-xs">Empty</div>}
              </div>
            </div>
          )
        })}
    ...
    return (
      <div>
        <div className="card mb-4">
          <h3 className="text-sm font-semibold text-gray-400 mb-2">Weekly Posting Schedule</h3>
          <p className="text-xs text-gray-500">{scheduled.length} scheduled / published items</p>
        </div>
        <div className="grid grid-cols-7 gap-2">
          {byDay.map(({ day, items }) => (
            <div key={day} className="card min-h-[150px]">
              <div className="text-xs text-gray-500 font-semibold mb-2 text-center">{day}</div>
              <div className="space-y-1">
                ...
                {items.length === 0 && <div className="text-center text-gray-700 text-[10px] py-4">—</div>}
              </div>
            </div>
          ))}
        </div>
        ...
        {content.filter((c) => !c.scheduled_date && !c.published_at).length > 0 && (
          <div className="card mt-4">
            <h3 className="text-sm font-semibold text-gray-400 mb-2">Unscheduled ({content.filter((c) => !c.scheduled_date && !c.published_at).length})</h3>
    ...
    <div className="card">
      <div className="space-y-2">
        {filtered.map((item) => {
          const s = stageOf(item.stage)
          return (
            <div key={item.id} className="flex items-center gap-3 p-3 bg-gray-800 rounded-lg cursor-pointer hover:bg-gray-750" onClick={() => setSelectedItem(item)}>
              <div className={`w-2.5 h-2.5 rounded-full shrink-0 ${s.dot}`} />
              <div className="flex-1 min-w-0">
                <div className="text-sm font-medium truncate">{item.title}</div>
                <div className="text-[10px] text-gray-500">{item.platform || "—"} · {item.scheduled_date || "Unscheduled"}</div>
              </div>
    ...
    <div className="space-y-6">
      ...
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-gray-400 text-sm mt-1">{content.length} items · {content.filter((c) => c.stage === "PUBLISHED").length} published</p>
        </div>
      ...
      <div className="flex gap-2 flex-wrap">
        <button onClick={() => setStageFilter("ALL")} className={`px-3 py-1.5 rounded text-xs ${stageFilter === "ALL" ? "bg-blue-600 text-white" : "bg-gray-800 text-gray-400"}`}>All</button>
        {stages.map((s) => (
          <button key={s.id} onClick={() => setStageFilter(s.id)} className={`px-3 py-1.5 rounded text-xs ${stageFilter === s.id ? `${s.bg} ${s.text}` : "bg-gray-800 text-gray-400"}`}>{s.label}</button>
        ))}
      </div>
      ...
      {/* Detail Modal */}
      {selectedItem && (
        <div className="fixed inset-0 bg-black/60 flex items-center justify-center z-50" onClick={() => { setSelectedItem(null); setEditingScript(null) }}>
          <div className="card max-w-lg w-full mx-4 max-h-[80vh] overflow-y-auto" onClick={(e) => e.stopPropagation()}>
            <div className="flex items-start justify-between mb-4">
              <div>
                ...
                <div className="flex items-center gap-2 mt-1">
                  ...
                  <span className="text-sm text-gray-400">{stageOf(selectedItem.stage).label}</span>
                  {selectedItem.platform && <span className="text-sm text-gray-500">· {platformIcons[selectedItem.platform.toLowerCase()] || ""} {selectedItem.platform}</span>}
                </div>
              </div>
              <button onClick={() => { setSelectedItem(null); setEditingScript(null) }} className="text-gray-500 hover:text-white text-xl">×</button>
            </div>

            {/* Script Section */}
            <div className="mb-4">
              <div className="flex items-center justify-between mb-2">
                <h3 className="text-sm font-semibold text-gray-400">Script</h3>
                <button
                  onClick={() => {
              ...
              {editingScript === selectedItem.id ? (
                <textarea
                  className="w-full h-40 bg-gray-800 rounded p-3 text-sm text-gray-300 resize-y font-mono"
                  value={scriptDraft}
                  onChange={(e) => setScriptDraft(e.target.value)}
                  autoFocus
                />
              ) : (
                <div className="bg-gray-800 rounded p-3 text-sm text-gray-300 whitespace-pre-wrap min-h-[60px]">
                  {selectedItem.script || <span className="text-gray-600 italic">No script yet</span>}
                </div>
              )}
            </div>
            ...
            {selectedItem.notes && (
              <div className="mb-4">
                ...
                <div className="text-sm text-gray-300">{selectedItem.notes}</div>
              </div>
            )}
