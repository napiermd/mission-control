    ...
    return (
      <div>
        ...
        <div className="card text-center py-12">
          <div className="text-hud-muted mb-4">No skill execution data yet</div>
  ...
  return (
    <div>
      ...
      <div className="grid grid-cols-4 gap-6 mb-8">
        <div className="card">
          <div className="text-hud-muted text-sm">Total Skills</div>
          <div className="text-3xl font-bold">{skills.length}</div>
          <div className="text-xs text-hud-muted mt-2">monitored</div>
        </div>
        <div className="card">
          <div className="text-hud-muted text-sm">Healthy</div>
          <div className="text-3xl font-bold text-green-400">{healthyCount}</div>
          <div className="text-xs text-hud-muted mt-2">≥90% success</div>
        </div>
        <div className="card">
          <div className="text-hud-muted text-sm">Warning</div>
          <div className="text-3xl font-bold text-amber-400">{warningCount}</div>
          <div className="text-xs text-hud-muted mt-2">70-89% success</div>
        </div>
        <div className="card">
          <div className="text-hud-muted text-sm">Critical</div>
          <div className="text-3xl font-bold text-red-400">{criticalCount}</div>
          <div className="text-xs text-hud-muted mt-2">&lt;70% success</div>
        </div>
      </div>
              ...
              <div
                ...
                className={`p-3 rounded cursor-pointer transition ${
                  selectedSkill === skill.skill_name
                    ...
                    : 'bg-space-dark hover:bg-space-panel'
                }`}
              >
                <div className="flex items-center justify-between mb-1">
                  <div className="font-medium">{skill.skill_name}</div>
                  <div className={`w-2 h-2 rounded-full ${
                    skill.status === 'healthy' ? 'bg-hud-green' :
                    skill.status === 'warning' ? 'bg-hud-amber' : 'bg-hud-red'
                  }`} />
                </div>
            ...
            <>
              ...
              <div className="card">
                ...
                <div className="grid grid-cols-3 gap-4">
                  <div>
                    <div className="text-hud-muted text-sm">Success Rate</div>
                    <div className={`text-2xl font-bold ${
                      skillStats.success_rate >= 90 ? 'text-green-400' :
                      skillStats.success_rate >= 70 ? 'text-amber-400' : 'text-red-400'
                    }`}>
                      {skillStats.success_rate.toFixed(1)}%
                    </div>
                  </div>
                  <div>
                    <div className="text-hud-muted text-sm">Avg Latency</div>
                    <div className="text-2xl font-bold">
                      {skillStats.avg_latency ? `${skillStats.avg_latency.toFixed(0)}ms` : 'N/A'}
                    </div>
                  </div>
                  <div>
                    <div className="text-hud-muted text-sm">Failures</div>
                ...
                <div className="card">
                  ...
                  <div className="space-y-3">
                    {patterns.map((pattern, idx) => (
                      <div key={idx} className="p-3 bg-red-900/30 border border-red-200 rounded">
                        <div className="flex items-center justify-between mb-2">
                          <div className="font-medium text-red-400">{pattern.error_type}</div>
                          <div className="text-sm text-hud-muted">{pattern.count} occurrences</div>
                        </div>
                        <div className="text-sm text-hud-text mb-1">
                  ...
                  <div className="space-y-2">
                    {Object.entries(skillStats.failure_types).map(([type, count]) => (
                      <div key={type} className="flex items-center justify-between">
                        <div className="text-hud-text">{type}</div>
                        <div className="flex items-center gap-2">
                          <div className="w-32 h-2 bg-space-border rounded overflow-hidden">
                            <div
                              className="h-full bg-hud-red"
                              style={{ width: `${(count / skillStats.failures) * 100}%` }}
                            />
                          </div>
                          <div className="text-sm text-hud-muted w-8 text-right">{count}</div>
                        </div>
                      </div>
                    ))}
                  </div>
                ...
                <div className="card">
                  ...
                  <div className="space-y-2">
                    {failures.map(failure => (
                      <div key={failure.id} className="p-3 bg-space-dark rounded text-sm">
                        <div className="flex items-center justify-between mb-1">
                          <div className="text-hud-muted">{failure.error_type}</div>
                          <div className="text-xs text-hud-muted">
